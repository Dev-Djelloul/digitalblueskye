...
## 12. Préférences profil front V1

`profile.html` expose une première version du centre de contrôle Digital Blue
Skye AI. Cette V1 est principalement front afin de ne pas modifier lourdement le
backend OAuth existant.

Préférences stockées localement dans `localStorage` :
...
Clé utilisée : `dbs_profile_preferences_cache`.

Champs disponibles :

- `projectStyle` : général, chef de projet digital, technique, produit,
  marketing, veille, portfolio.
- `favoriteFormat` : texte structuré, tableau, checklist, plan d'action,
  roadmap, synthèse, livrable HTML autonome.
- `detailLevel` : court, équilibré, approfondi, expert.
- `preferredLanguage` : français, anglais, bilingue FR/EN.
- `tone` : standard, pédagogique, direct, expert, créatif, stratégique.
- `companion` : Skye, Pilot, Builder, Scout, Muse, Guardian.

`scripts/dbs-auth.js` expose `getAssistantPreferences()`,
`saveAssistantPreferences()` et `clearLocalProfileData()`. Les valeurs sont
validées côté navigateur contre des listes fermées. `tone` est aussi envoyé à
`PATCH /auth/profile` car le backend sait déjà le persister ; les autres champs
restent en cache local pour cette V1.

`scripts/ai-assistant.js` joint ces préférences à `payload.preferences` pour
personnaliser les réponses. Ces préférences sont indicatives et forgeables :
elles ne doivent jamais être utilisées comme preuve de sécurité.

Actions de données locales :

- suppression de `dbs_profile_preferences_cache` ;
- suppression des anciens caches `dbs_user_session` et `dbs_user_profile` ;
- conservation de la session serveur, sauf clic explicite sur déconnexion.

Les sections `Utilisation IA` et `Historique` affichent des états honnêtes
`Bientôt disponible` quand la donnée backend n'existe pas encore. Elles seront
reliées plus tard à des endpoints dédiés, par exemple `GET /auth/usage` ou
`GET /ai/usage`, et aux tables `ai_usage_events` / `user_sessions`.

Facebook reste affiché comme différé lorsque `/auth/providers` renvoie
`facebook: false`, le temps de la vérification Meta.

Cette V1 front prépare l'expérience profil avancée. Les sections Usage IA et
Historique seront reliées à des endpoints backend lors d'une prochaine étape.
